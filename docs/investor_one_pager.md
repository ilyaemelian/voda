# VODA Investor One-Pager

Date: 2026-05-09

## Company

**VODA** is an international subscription platform that helps students discover academic opportunities, track deadlines, build achievements, and strengthen their university admissions profile.

The product starts with Russian olympiads and expands globally into academic competitions, contests, science fairs, scholarships, summer programs, and admissions profile planning.

## One-Line Pitch

**VODA turns scattered student opportunities into personalized admissions routes.**

## Brand Meaning

VODA is a short international brand with a built-in acronym layer:

- **V** - Victory
- **O** - Olympiad at launch, expanding into Opportunities internationally
- **D** - Drive and Direction
- **A** - Achievement and Advantage

In Russian, **voda** means water: natural, memorable, essential, and associated with flow. The brand uses this metaphor to describe how VODA turns fragmented academic opportunities into a clear route.

The domain **voda.to** reinforces direction: VODA to admission, VODA to opportunity, VODA to achievement, VODA to the top.

## Mission and Ecosystem Context

VODA is part of a broader education and admissions vision connected to the EvoStudy ecosystem.

The larger mission is to help ambitious students move through the full path from selective high school admissions to university admissions, academic achievement, scholarships, and long-term career opportunities.

VODA focuses on the opportunity layer: competitions, deadlines, achievements, and admissions routes. It complements broader admissions products, university search tools, checklists, and student guidance services for families targeting the US, UK, and other top education markets.

Our mission is to help young people find their route, build real achievements, and turn ambition into education and career opportunities.

## Problem

Families miss valuable academic opportunities because information is fragmented across school chats, university pages, official competition websites, social media, tutors, and admissions rules.

In Russia, missing an olympiad deadline can mean losing direct admission benefits, extra points, or portfolio value.

In the US and other international markets, students often miss competitions, research programs, science fairs, summer programs, and scholarships that could strengthen college applications.

## Solution

VODA is a student opportunity navigator.

The platform helps families:

- discover relevant olympiads, contests, competitions, programs, and scholarships;
- track registration and stage deadlines;
- understand why each opportunity matters;
- build a year-by-year student route;
- collect achievements into an admissions-ready profile;
- receive reminders, reports, and next-step guidance.

## Product Status

VODA has public-facing materials, market framing, landing pages, pricing strategy, and a product roadmap.

The public package communicates the product thesis, mission, market direction, and investor-facing narrative.

## Business Model

VODA is a recurring subscription business.

Russia:

- Start: 1,990 RUB/month
- Path: 2,990 RUB/month
- Pro: 3,990 RUB/month

United States:

- Start: $19/month
- Path: $39/month
- Pro: $79/month

Additional revenue streams:

- family plans;
- annual plans;
- profile audits;
- target university strategy;
- parent consultations;
- tutor and school dashboards;
- B2B school/counselor licenses.

## Go-To-Market

Russia:

- Yandex Search;
- VK Ads targeting parents;
- Telegram channel placements;
- tutor partnerships;
- olympiad and admissions SEO;
- parent webinars.

United States:

- Google Search;
- Meta Ads targeting parents, not minors;
- parent Facebook groups;
- homeschool and gifted education communities;
- counselors and tutors;
- SEO around competitions, scholarships, and summer programs.

## Milestones

0-30 days:

- launch Russian landing and Telegram preview flow;
- launch US landing and waitlist;
- test pricing and messaging;
- collect first leads and interviews;
- prepare product demo video.

31-90 days:

- convert first paid users in Russia;
- run US pre-sale or concierge beta;
- add parent payment link;
- add recurring payment infrastructure;
- collect CAC, conversion, retention, and paid intent data.

6 months:

- reach operational validation in Russia;
- prove repeatable acquisition channels;
- build investor data room;
- prepare seed/investor outreach in the US market.

## Vision

VODA can become the global opportunity layer for ambitious students:

**from competitions and deadlines to achievements, scholarships, and admissions outcomes.**

The long-term goal is to build a profitable subscription business with potential for a $4M-$20M valuation or strategic exit.

## Legal and IP Note

This document is a high-level investor and concept summary created for timestamping, investor preparation, and public-safe communication.

Product development, operational workflows, and fundraising materials are managed through the project team.

This document is not a legal filing, patent filing, trademark filing, securities offering, or investment solicitation.
